		...
		switch ($command) {
			...
			case 'cycle':
				...
				if ($cycles === 1) {
					...
					foreach ($results as $i => $result) {
						$cycleNum = $i + 1;
						echo
							"    Cycle {$cycleNum}: {$result['days']} days, "
			...
			case 'repay':
				...
				if ($loan->isFullyPaid()) {
					...
					$totalPaid = $status['principal']['paid']->add($totalRevenue)->add(
						$status['overpayment']['balance'],
					);
					...
					echo
						'    (Interest: '
							. $status['interest']['paid']->format()
							. ', Fees: '
							. $status['fees']['paid']->format()
							. ")\n"
					;
